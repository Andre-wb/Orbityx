...
/**
 * Supported bucketing intervals. Used by floorToInterval and UI selectors.
 */
export type Interval =
  | '1m' | '2m' | '5m' | '10m' | '15m' | '20m' | '25m' | '30m' | '45m'
  | '1h' | '2h' | '4h' | '6h' | '12h' | '16h'
  | '1d' | '3d'
  | '1w' | '2w'
 ...
 */
export function parseISO(iso: number | Date | string): number {
 ...
 */
export function format(ts: number, pattern: string = 'YYYY-MM-DD HH:mm:ss'): string {
 ...
 */
export function floorToInterval(ts: number, interval: Interval): number {
