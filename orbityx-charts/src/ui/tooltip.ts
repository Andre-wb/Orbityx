/**
 * Normalized OHLCV candle used by the tooltip.
 */
type Candle = {
    timestamp: number;
    open: number;
...
/**
 * Minimum surface of the chart engine the tooltip relies on.
 * (Canvas handle, current visible candles, and geometry config.)
 */
type ChartEngineLike = {
 ...
 */
export function initTooltip(chartEngine: ChartEngineLike): HTMLDivElement {
  ...
  // Cache references to frequently updated DOM nodes.
    const elements = {
        date: tooltip.querySelector('.tooltip-date') as HTMLDivElement | null,
        time: tooltip.querySelector('.tooltip-time') as HTMLDivElement | null,
        open: tooltip.querySelector('#tooltip-open') as HTMLSpanElement | null,
        high: tooltip.querySelector('#tooltip-high') as HTMLSpanElement | null,
        low: tooltip.querySelector('#tooltip-low') as HTMLSpanElement | null,
        close: tooltip.querySelector('#tooltip-close') as HTMLSpanElement | null,
        volume: tooltip.querySelector('#tooltip-volume') as HTMLSpanElement | null,
        volumeFill: tooltip.querySelector('.volume-fill') as HTMLDivElement | null,
        indicators: tooltip.querySelector('#tooltip-indicators') as HTMLDivElement | null,
    };
  ...
  // Track the last-rendered candle index to avoid redundant DOM updates.
    ...
    canvas.addEventListener('mousemove', (e: MouseEvent) => {
        ...
        if (candleIndex === -1 || candleIndex >= chartEngine.state.visibleData.length) {
            (tooltip as HTMLDivElement).style.display = 'none';
            activeCandleIndex = -1;
            return;
        }
 ...
 */
    function findCandleAtPosition(x: number): number {
 ...
 */
    function updateTooltipContent(candleIndex: number): void {
        ...
        if (elements.volumeFill) elements.volumeFill.style.width = `${volumePercent}%`;

        updateIndicators();
        (tooltip as HTMLDivElement).style.display = 'block';
    }
 ...
 */
    function positionTooltip(mouseX: number, mouseY: number): void {
        const tooltipWidth = (tooltip as HTMLDivElement).offsetWidth;
        const tooltipHeight = (tooltip as HTMLDivElement).offsetHeight;
        ...
        (tooltip as HTMLDivElement).style.left = `${posX}px`;
        (tooltip as HTMLDivElement).style.top = `${posY}px`;
    }
