...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (intro_binance_picture)
        intro_binance_picture.src = startDark ? dark_binance_picture : light_binance_picture;
    if (intro_planet)
        intro_planet.src = startDark ? dark_planet : light_planet;
    if (!toggle)
        return;
    ...
    toggle.addEventListener('change', () => {
        ...
        if (logo)
            switchImage(logo, newLogoSrc);
        if (intro_binance_picture)
            switchImage(intro_binance_picture, newBinanceSrc);
        if (intro_planet)
            switchImage(intro_planet, newPlanetSrc);
    });
});
