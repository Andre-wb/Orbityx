...
export class BinanceProvider {
    ...
    async fetchPage(symbol, interval, limit, endTime, startTime) {
    ...
    async fetchCandles(req) {
        ...
        if (req.to !== undefined) {
            const rows = await this.fetchPage(req.instrumentId, interval, limit, req.to, req.from);
        ...
        while (rows.length === limit && page < this.INITIAL_PAGES) {
            ...
            if (!firstRow)
                break;
            const oldestTs = firstRow[0];
            rows = await this.fetchPage(req.instrumentId, interval, limit, oldestTs - 1);
            if (!rows.length)
                break;
    ...
    async fetchMarketStats(instrumentId) {
        const res = await fetch(`${this.base}/ticker/24hr?symbol=${encodeURIComponent(instrumentId)}`, { signal: AbortSignal.timeout(this.timeout) });
        if (!res.ok)
