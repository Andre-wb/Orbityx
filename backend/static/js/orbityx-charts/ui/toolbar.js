 ...
 */
export function initToolBar(onTimeframeChange, onChartTypeChange, engine) {
...
// Event Wiring
// ─────────────────────────────────────────────────────────────────────────────
function wireEvents(toolbar, onTfChange, onCtChange, engine) {
    // Timeframe buttons.
    const tfBtns = toolbar.querySelectorAll('[data-tf]');
    tfBtns.forEach((btn) => {
    ...
    // Chart type buttons.
    const ctBtns = toolbar.querySelectorAll('[data-ct]');
    ctBtns.forEach((btn) => {
        btn.addEventListener('click', () => {
            ctBtns.forEach((b) => b.classList.remove('active'));
            btn.classList.add('active');
            onCtChange(btn.dataset.ct ?? 'candlestick');
        });
    });
    // Drawing tools.
    const drawBtns = toolbar.querySelectorAll('[data-draw]');
    drawBtns.forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (alreadyActive) {
                ...
                btn.classList.add('active');
                engine.setDrawingMode(btn.dataset.draw ?? 'none');
            }
        });
    });
    // Clear drawings.
    ...
    const indDropdown = toolbar.querySelector('#indicator-dropdown');
    ...
    toolbar.querySelectorAll('[data-indicator]').forEach((chk) => {
        chk.addEventListener('change', () => {
            const id = chk.dataset.indicator;
            engine.toggleIndicator(id);
        });
    });
    // ── Scale toggle ──────────────────────────────────────────────────────
    const scaleBtn = toolbar.querySelector('#scale-toggle');
    if (scaleBtn) {
        const scaleLabels = { linear: 'Lin', logarithmic: 'Log', percentage: '%' };
        scaleBtn.addEventListener('click', () => {
            engine.cycleScaleType();
            const svg = scaleBtn.querySelector('text');
            if (svg)
                svg.textContent = scaleLabels[engine.state.scaleType];
    ...
    const replayBtns = ['replay-step-back', 'replay-pause', 'replay-step-fwd', 'replay-faster']
        .map(id => toolbar.querySelector(`#${id}`));
    const setReplayBtnsDisabled = (disabled) => {
        replayBtns.forEach(b => { if (b)
            b.disabled = disabled; });
    };
    ...
    // ── Layout buttons ────────────────────────────────────────────────────
    const layoutBtns = toolbar.querySelectorAll('[data-layout]');
