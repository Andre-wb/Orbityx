...
function run(source, n = 3) {
    const tokens = tokenize(source);
    const ast = parse(tokens);
    return interpret(ast, makeCandles(n));
}
function lastPlot(source, n = 3) {
    const result = run(source, n);
    const plot = result.outputs.find(o => o.type === 'plot');
...
describe('Macro: assert!', () => {
    ...
    it('assert! uses custom message', () => {
        let msg = '';
        try {
            run(`assert!(false, "my custom error");`);
        }
        catch (e) {
            msg = e.message;
        }
