...
// ─── Expr ──────────────────────────────────────────────────────────────────
export function parseExpr(ts, minPrec = 0, deps) {
    ...
    while (true) {
        ...
        if (op === '..' || op === '..=') {
            ...
            left = { kind: 'BinaryExpr', op: op, left, right, line, col };
        }
    }
    return left;
}
function currentBinaryOp(ts) {
    const t = ts.current();
    switch (t.type) {
...
function parseUnary(ts, deps) {
    const { line, column: col } = ts.current();
    if (ts.at('MINUS')) {
        ts.advance();
        const operand = parseUnary(ts, deps);
        return { kind: 'UnaryExpr', op: '-', operand, line, col };
    }
    if (ts.at('BANG')) {
        ts.advance();
        const operand = parseUnary(ts, deps);
    ...
    while (true) {
        ...
        if (ts.at('LBRACKET')) {
            ...
            if (node.kind === 'Identifier' && (index.kind === 'NumberLit' || index.kind === 'Identifier')) {
                node = { kind: 'HistoryRefExpr', series: node, offset: index, line, col };
            }
            else {
...
function parsePrimary(ts, deps) {
    const t = ts.current();
    const { line, column: col } = t;
    if (ts.at('NUMBER')) {
        ts.advance();
        return { kind: 'NumberLit', value: parseFloat(t.value), line, col };
    }
    if (ts.at('STRING')) {
        ts.advance();
        return { kind: 'StringLit', value: t.value, line, col };
    }
    if (ts.at('TRUE')) {
        ts.advance();
        return { kind: 'BoolLit', value: true, line, col };
    }
    if (ts.at('FALSE')) {
        ts.advance();
        return { kind: 'BoolLit', value: false, line, col };
    }
    if (ts.at('COLOR')) {
        ts.advance();
        return { kind: 'ColorLit', hex: t.value, line, col };
    }
    if (ts.at('NONE')) {
        ts.advance();
        return { kind: 'NoneLit', line, col };
    }
    if (ts.at('SOME')) {
        ts.advance();
        ts.expect('LPAREN');
    ...
    if (ts.at('SELF')) {
        ts.advance();
        return { kind: 'SelfExpr', line, col };
    }
    if (ts.at('PIPE')) {
        return parseClosure(ts, line, col, deps);
    }
    if (ts.at('IF')) {
        return parseIfExpr(ts, deps);
    }
    if (ts.at('MATCH')) {
        return parseMatchExpr(ts, deps);
    }
    if (ts.at('LBRACE')) {
        return parseBlockExpr(ts, deps);
    }
    if (ts.at('LBRACKET')) {
        ts.advance();
        const elements = [];
...
function parseIdentOrCall(ts, line, col, deps) {
    ...
    if (ts.at('LBRACE'))
        return parseStructInit(ts, name, line, col, deps);
    return { kind: 'Identifier', name, line, col };
}
function parseStructInit(ts, typeName, line, col, deps) {
    ts.expect('LBRACE');
    const fields = [];
...
function parseIfExpr(ts, deps) {
    ...
    if (ts.tryConsume('ELSE')) {
        if (ts.at('IF')) {
            ...
            return { kind: 'IfExpr', condition, thenBranch, elseBranch: nested, line, col };
        }
...
function parseInlineBlock(ts, deps) {
    ts.expect('LBRACE');
    const stmts = [];
    while (!ts.at('RBRACE') && !ts.at('EOF'))
        stmts.push(parseStatement(ts, { parseExpr: (t, p) => parseExpr(t, p, deps) }));
    ts.expect('RBRACE');
    return stmts;
}
function parseMatchExpr(ts, deps) {
    const { line, column: col } = ts.current();
    ts.expect('MATCH');
    const value = parseExpr(ts, 0, deps);
    ts.expect('LBRACE');
...
function parseMatchPattern(ts) {
    ...
    if (ts.at('NUMBER')) {
        const v = { kind: 'NumberLit', value: parseFloat(t.value), line: t.line, col: t.column };
        ts.advance();
        return { kind: 'LiteralPattern', value: v };
    }
    if (ts.at('STRING')) {
        const v = { kind: 'StringLit', value: t.value, line: t.line, col: t.column };
        ts.advance();
        return { kind: 'LiteralPattern', value: v };
    }
    if (ts.at('TRUE')) {
        ts.advance();
        return { kind: 'LiteralPattern', value: { kind: 'BoolLit', value: true, line: t.line, col: t.column } };
    }
    if (ts.at('FALSE')) {
        ts.advance();
        return { kind: 'LiteralPattern', value: { kind: 'BoolLit', value: false, line: t.line, col: t.column } };
    }
    if (ts.at('COLOR')) {
        const v = { kind: 'ColorLit', hex: t.value, line: t.line, col: t.column };
        ts.advance();
        return { kind: 'LiteralPattern', value: v };
    }
    if (ts.at('IDENT')) {
...
export function parseBlockExpr(ts, deps) {
    const { line, column: col } = ts.current();
    ts.expect('LBRACE');
    const body = [];
    let tail = null;
    const stmtDeps = { parseExpr: (t, p) => parseExpr(t, p, deps) };
    while (!ts.at('RBRACE') && !ts.at('EOF')) {
...
function parseClosure(ts, line, col, deps) {
    ts.expect('PIPE');
    const params = [];
