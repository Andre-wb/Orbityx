import { atr as atrCalc, adx as adxCalc, bollingerBands, keltnerChannels, donchianChannels, parabolicSAR as psarCalc, ichimoku as ichimokuCalc, } from '../../../utils/math.js';
...
export const entries = [
    ...
    ['adx', (args, env, line, col) => {
            ...
            return { __type: 'struct', __name: 'ADXResult', fields: { adx: (env.seriesCache.get(key + ':adx') ?? [])[i] ?? NaN, plus_di: (env.seriesCache.get(key + ':plusDI') ?? [])[i] ?? NaN, minus_di: (env.seriesCache.get(key + ':minusDI') ?? [])[i] ?? NaN } };
        }],
    ['bollinger', (args, env, line, col) => {
            ...
            const i = env.barIndex;
            return { __type: 'struct', __name: 'BandsResult', fields: { upper: (env.seriesCache.get(key + ':u') ?? [])[i] ?? NaN, middle: (env.seriesCache.get(key + ':m') ?? [])[i] ?? NaN, lower: (env.seriesCache.get(key + ':l') ?? [])[i] ?? NaN } };
        }],
    ...
    ['ichimoku', (args, env, line, col) => {
            ...
            const i = env.barIndex;
            const g = (k) => (env.seriesCache.get(k) ?? [])[i] ?? NaN;
            return { __type: 'struct', __name: 'IchimokuResult', fields: { tenkan: g(key + ':tk'), kijun: g(key + ':kj'), senkou_a: g(key + ':sa'), senkou_b: g(key + ':sb2'), chikou: g(key + ':ck') } };
        }],
];
