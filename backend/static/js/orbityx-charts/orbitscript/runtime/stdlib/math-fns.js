...
export const entries = [
    ...
    ['max', (args, _, line, col) => { if (Array.isArray(args[0]))
            return Math.max(...args[0]); return Math.max(asNum(args[0], 'max:a', line, col), asNum(args[1], 'max:b', line, col)); }],
    ['min', (args, _, line, col) => { if (Array.isArray(args[0]))
            return Math.min(...args[0]); return Math.min(asNum(args[0], 'min:a', line, col), asNum(args[1], 'min:b', line, col)); }],
...
//# sourceMappingURL=math-fns.js.map