...
export function isOSColor(v) {
    return typeof v === 'object' && v !== null && !Array.isArray(v) &&
        '__type' in v === false && 'r' in v && 'hex' in v;
}
export function osValueToString(v) {
    if (v === undefined)
        return 'undefined';
    if (v === null)
        return 'None';
    ...
    if (isOSColor(v))
        return v.hex;
    if (Array.isArray(v))
        return '[' + v.map(osValueToString).join(', ') + ']';
    if (typeof v === 'object' && '__type' in v) {
        const obj = v;
        if (obj.__type === 'struct')
            return `${obj.__name} { ${Object.entries(obj.fields).map(([k, fv]) => `${k}: ${osValueToString(fv)}`).join(', ')} }`;
        if (v.__type === 'enum') {
            const e = v;
