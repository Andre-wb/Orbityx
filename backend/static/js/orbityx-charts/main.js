...
import { initLegend, applyMarketStats, populateSymbolSelector, } from './ui/legend.js';
...
class OrbityxChart {
    ...
    async init() {
        if (registry.instrumentCount === 0) {
            console.warn('[Orbityx] No instruments registered.\n' +
        ...
        try {
            ...
            initToolBar(tf => this.handleTimeframeChange(tf), ct => this.engine.setChartType(ct), this.engine);
            ...
            this.engine.onAlertTriggered = (alert) => {
    ...
    async loadMoreHistory() {
        ...
        try {
            ...
            const added = await dataManager.prependCandles(this.activeId, this.timeframe, oldest.timestamp - 1);
            if (added > 0) {
                ...
                // Provider returned nothing — we've reached the beginning of
                // available history. Keep fetchingMoreHistory=true to prevent
                // the engine from firing the hook again (reset on symbol/TF change).
            ...
            // Only release the engine gate when there is more data to load.
            if (!historyExhausted) {
                this.engine.fetchingMoreHistory = false;
            }
        }
    ...
    async refreshStats() {
        ...
        try {
            ...
            if (stats)
                applyMarketStats(stats, this.activeId);
        }
        catch {
    ...
    // ── WebSocket ─────────────────────────────────────────────────────────────
    setupWebSocket() {
        this.wsUnsub = wsService.subscribe((msg) => {
            switch (msg.type) {
                case 'candle':
                    if (msg.payload) {
    ...
    showError(title, message) {
        ...
        if (!el) {
            console.error(`[Orbityx] ${title}: ${message}`);
            return;
        }
        ...
        el.querySelector('.error-dismiss')?.addEventListener('click', () => { el.style.display = 'none'; }, { once: true });
    }
    toggleFullscreen = () => {
     ...
     */
    async addCompare(instrumentId, color = '#f59e0b') {
        try {
    ...
    startFPSMonitor() {
        ...
        const tick = () => {
            ...
            if (el)
                el.textContent = `${Math.round(1_000 / (now - last))} FPS`;
            last = now;
...
window['OrbityxChart'] = OrbityxChart;
