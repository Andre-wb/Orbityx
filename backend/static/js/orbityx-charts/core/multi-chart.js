 ...
 */
export function applyLayout(containerId, layout) {
 ...
 */
export function resetLayout(containerId) {
    const container = document.getElementById(containerId);
    if (!container)
        return;
    container.querySelectorAll('.mc-cell').forEach(el => el.remove());
 ...
 */
export function syncCrosshair(engines, sourceIndex) {
    const src = engines[sourceIndex];
    if (!src || !src.state.mouseInside)
        return;
