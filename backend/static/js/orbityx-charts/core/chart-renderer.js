import { priceToY as vpPriceToY, indexToX as vpIndexToX, xToIndex as vpXToIndex, yToPrice as vpYToPrice, timestampToX as vpTimestampToX, candleStep, volumePanelH, } from './viewport.js';
...
export function renderError(r, err) {
    ...
    ctx.fillText(String(err instanceof Error ? err.message : err).slice(0, 80), state.width / 2, state.height / 2 + 24);
}
// ─────────────────────────────────────────────────────────────────────────────
// Background
// ─────────────────────────────────────────────────────────────────────────────
...
function drawWatermark(r) {
    ...
    // SVG logo path: M3 18 L9 12 L13 16 L21 6  (viewBox 0 0 24 24)
    // Scale to ~120px wide (factor = 5), then centre it
    const S = 5;
    const pts = [[3,18],[9,12],[13,16],[21,6]];
    const logoW = 21 * S;  // 105
    const logoH = 18 * S;  // 90
    const ox = cx - logoW / 2;
    const oy = cy - logoH / 2 - 18;  // nudge up slightly to leave room for text
    ctx.beginPath();
    pts.forEach(([x, y], i) => {
        const px = ox + x * S;
        const py = oy + y * S;
    ...
    ctx.lineJoin    = 'round';
    ctx.stroke();
    // "ORBITYX" wordmark below the logo
    ctx.fillStyle    = '#ffffff';
    ctx.textAlign    = 'center';
    ctx.textBaseline = 'top';
    ctx.font         = '700 18px "Space Grotesk", system-ui, sans-serif';
    ctx.fillText('ORBITYX', cx, oy + logoH + 10);
    ctx.restore();
}
// ─────────────────────────────────────────────────────────────────────────────
// Grid
...
function drawTimeAxis(r) {
    ...
    for (let i = 0; i < state.visibleData.length; i += labelStep) {
        ...
        if (x < plot.left + 20 || x > plot.right - 20)
            continue;
...
// Current Price Line
// ─────────────────────────────────────────────────────────────────────────────
function drawCurrentPriceLine(r) {
    const { ctx, colors, state, plot, config } = r;
...
// Volume Panel
// ─────────────────────────────────────────────────────────────────────────────
function drawVolumePanelIfActive(r) {
    if (!r.state.activeIndicators.has('volume'))
        return;
    drawVolumePanel(r);
}
export function drawVolumePanel(r) {
    const { ctx, colors, state, plot, config } = r;
    if (!state.visibleData.length)
        return;
...
// Indicator Overlays
// ─────────────────────────────────────────────────────────────────────────────
function drawIndicatorOverlays(r) {
...
function drawLineSeries(r, points, color) {
    const { ctx, state } = r;
    ctx.save();
...
function drawBollingerBands(r, series) {
    ...
    const drawBand = (getter, color, dash) => {
        ...
        for (let i = 0; i < state.visibleData.length; i++) {
            ...
            if (!pt || !isFiniteNumber(getter(pt))) {
                started = false;
                continue;
            }
            const x = indexToX(r, i) + halfW(r);
    ...
    const fillColor = seriesColor.includes('rgb')
    ...
    for (let i = 0; i < state.visibleData.length; i++) {
        const pt = pts[state.viewportStart + i];
        if (!pt || !isFiniteNumber(pt.upper)) {
            started = false;
            continue;
        }
        const x = indexToX(r, i) + halfW(r);
        if (!started) {
            ctx.moveTo(x, priceToY(r, pt.upper));
            started = true;
        }
        else {
            ctx.lineTo(x, priceToY(r, pt.upper));
        }
    }
    for (let i = state.visibleData.length - 1; i >= 0; i--) {
        const pt = pts[state.viewportStart + i];
        if (!pt || !isFiniteNumber(pt.lower))
            continue;
        const x = indexToX(r, i) + halfW(r);
...
function drawSubPanel(r, series, panelTop) {
    ...
    if (series.subPanelRange) {
        ...
        for (let i = 0; i < state.visibleData.length; i++) {
            ...
            if (pt)
                vals.push(pt.macd, pt.signal, pt.histogram);
        }
    ...
    const toY = (v) => panelTop + panelH - ((v - rangeMin) / (rangeMax - rangeMin)) * panelH;
    if (series.id === 'rsi_14') {
    ...
    if (series.macdPoints) {
        ...
        for (let i = 0; i < state.visibleData.length; i++) {
            const pt = series.macdPoints[state.viewportStart + i];
            if (!pt || !isFiniteNumber(pt.histogram))
                continue;
            const x = indexToX(r, i);
        ...
        for (let i = 0; i < state.visibleData.length; i++) {
            const pt = series.macdPoints[state.viewportStart + i];
            if (!pt || !isFiniteNumber(pt.signal)) {
                s = false;
                continue;
            }
            const x = indexToX(r, i) + bw / 2;
...
// Drawing Tools
// ─────────────────────────────────────────────────────────────────────────────
function drawDrawings(r) {
    const all = [...r.drawings, ...(r.draftDrawing ? [r.draftDrawing] : [])];
    for (const drawing of all)
        renderDrawing(r, drawing);
}
function renderDrawing(r, drawing) {
    ...
    switch (drawing.type) {
        ...
        case 'pitchfork': {
            ...
            ctx.beginPath();
            ctx.moveTo(xA, yA);
            ctx.lineTo(xA + (dx / len) * ext, yA + (dy / len) * ext);
            ctx.stroke();
            // Upper prong (from B)
            ctx.beginPath();
            ctx.moveTo(xB, yB);
            ctx.lineTo(xB + (dx / len) * ext, yB + (dy / len) * ext);
            ctx.stroke();
            // Lower prong (from C)
            ctx.beginPath();
            ctx.moveTo(xC, yC);
            ctx.lineTo(xC + (dx / len) * ext, yC + (dy / len) * ext);
            ctx.stroke();
        ...
        case 'measure': {
            ...
            ctx.beginPath();
            ctx.roundRect(midX - tw / 2, midY - 12, tw, 20, 4);
            ctx.fill();
...
// Hollow Candlesticks
// ─────────────────────────────────────────────────────────────────────────────
function drawHollowCandlesticks(r) {
    const { ctx, colors, state } = r;
    const bw = bodyW(r);
    ctx.save();
    for (let i = 0; i < state.visibleData.length; i++) {
...
// ─────────────────────────────────────────────────────────────────────────────
function drawCompareSeries(r) {
    if (!r.compareSeries.length || !r.state.visibleData.length)
        return;
    const { ctx, state, plot } = r;
    const hw = halfW(r);
...
// Scale & Magnet Badges
// ─────────────────────────────────────────────────────────────────────────────
function drawScaleBadge(r) {
    if (r.state.scaleType === 'linear')
        return;
    const { ctx, plot } = r;
...
function drawMagnetBadge(r) {
    if (!r.state.magnetEnabled)
        return;
    const { ctx, plot } = r;
    ctx.save();
