...
import { computePlotRect, computeVisibleData, candleStep, maxVisibleCandles, priceToY, yToPrice, indexToX, xToIndex, volumePanelH, } from './viewport.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe('computePlotRect', () => {
    ...
    it('shrinks for volume panel', () => {
        const config = makeConfig();
        const state = makeState({ activeIndicators: new Set(['volume']) });
        const plot = computePlotRect(config, state, 0);
