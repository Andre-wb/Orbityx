...
function smiCalc(src: readonly number[], period: number, smoothPeriod: number): number[] {
    const n = src.length;
...
function elderForceCalc(close: readonly number[], volume: readonly number[], period: number): number[] {
    const n = close.length;
    const forceRaw: number[] = new Array(n).fill(NaN);
    for (let i = 1; i < n; i++) {
